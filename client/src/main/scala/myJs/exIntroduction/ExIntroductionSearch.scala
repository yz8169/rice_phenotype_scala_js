...
/**
  ...
  */
@JSExportTopLevel("ExIntroductionSearch")
object ExIntroductionSearch {
  var i = 0
  var j = 0
  var array: js.Array[js.Dictionary[String]] = _
  val samples = Shared.exSelectPhenotypes
  val phenotypeMap=Shared.exIntroductionMap

  @JSExport("init")
  def init = {
    ExIntroductionView.refreshTableView
    refreshTableData()
    bootStrapValidator
    refreshSelect
    ...
    val html = div(cls := "form-group", id := s"select${i}",
      ...
      div(cls := "col-sm-2",
        select(cls := "form-control", name := "phenotypeNames[]", id := s"phenotype${i}",
          onclick := s"ExIntroductionSearch.change(this,'content${i}')", raw(phenotypeHtml)),
      ),
      div(id := s"content${i}", raw(htmlAfter)),
      div(cls := "col-sm-2",
        button(`type` := "button", cls := "btn btn-default deleteButton", width := "100%", onclick := s"ExIntroductionSearch.deleteSelect('${i}')", "删除此条件"),
      ),
    ).render
    $("#content").append(html)
    setDisable
