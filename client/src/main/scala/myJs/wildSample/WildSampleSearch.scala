...
/**
  ...
  */
@JSExportTopLevel("WildSampleSearch")
object WildSampleSearch {
  var i = 0
  var j = 0
  ...
  @JSExport("init")
  def init = {
    WildSampleView.refreshTableView
    refreshTableData()
    bootStrapValidator
    refreshSelect
    ...
    val html = div(cls := "form-group", id := s"select${i}",
      ...
      div(cls := "col-sm-2",
        select(cls := "form-control", name := "phenotypeNames[]", id := s"phenotype${i}",
          onclick := s"WildSampleSearch.change(this,'content${i}')", raw(phenotypeHtml)),
      ),
      div(id := s"content${i}", raw(htmlAfter)),
      div(cls := "col-sm-2",
        button(`type` := "button", cls := "btn btn-default deleteButton", width := "100%", onclick := s"WildSampleSearch.deleteSelect('${i}')", "删除此条件"),
      ),
    ).render
    $("#content").append(html)
    setDisable
