...
/**
  ...
  */
@JSExportTopLevel("ExIntroductionUpdate")
object ExIntroductionUpdate {

  @JSExport("init")
  def init = {
    ExIntroductionAddMan.refreshPhenotypeView
    fillValue
    bootStrapValidator
  ...
  def fillValue = {
    ...
    val url = g.jsRoutes.controllers.ExIntroductionController.getSamplePhenotypeInfo().url.toString
  ...
  def update = {
    ...
    if (valid) {
      val data = ExIntroductionAddMan.getData
      val url = g.jsRoutes.controllers.adminC.ExIntroductionController.update().url.toString
      val ajaxSettings = JQueryAjaxSettings.url(url).`type`("post").data(JSON.stringify(data)).
        contentType("application/json").success { (data, status, e) =>
