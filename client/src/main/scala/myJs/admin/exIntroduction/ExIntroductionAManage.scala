...
/**
  ...
  */
@JSExportTopLevel("ExIntroductionAManage")
object ExIntroductionAManage {

  val dir="ex_introduction"
          ...
          div(cls := "col-sm-4",
            div(cls := "thumbnail",
              ...
              div(cls := "caption",
                ...
                h5("说明：", desc,
                  raw("&nbsp;"),
                  a(title := "更改说明", onclick := s"ExIntroductionAManage.updateDescBefore('${key}','${value}')", cursor.pointer,
                    span(
                      em(cls := "fa fa-edit")
                    )
                  )
                ...
                p(
                  a(cls := "btn btn-primary", role := "button", onclick := s"ExIntroductionAManage.show('${number}','${key}','${dir}')", "查看"), raw("&nbsp;"),
                  a(cls := "btn btn-default", role := "button", onclick := s"ExIntroductionAManage.delete('${number}','${key}')", "删除"), raw("&nbsp;"),
                )
              )
            )
          )
  ...
  def delete(number: String, fileName: String) = {
    ...
    g.swal(dict, () => {
      val url = g.jsRoutes.controllers.adminC.ExIntroductionController.deleteAppendix().url.toString
  ...
  @JSExport("updateDesc")
  def updateDesc = {
    val url = g.jsRoutes.controllers.adminC.ExIntroductionController.updateDesc().url.toString
    val data = js.Dictionary(
