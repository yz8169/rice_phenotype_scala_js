...
/**
  ...
  */
@JSExportTopLevel("ExIntroductionDetail")
object ExIntroductionDetail {
          ...
          div(cls := "col-sm-4",
            div(cls := "thumbnail",
              ...
              div(cls := "caption",
                ...
                h5("说明：", desc),
                p(
                  a(cls := "btn btn-primary", role := "button",
                    onclick := s"LocalSampleAManage.show('${number}','${key}','${ExIntroductionAManage.dir}')", "查看"),
                  raw("&nbsp;"),
                )
              )
            )
